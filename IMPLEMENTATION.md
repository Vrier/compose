...
**S10 as-built notes:** (a) `toLaTeX`/`typeToLaTeX` live in engine.js as exact structural mirrors of `toStr` (same `prec` bracketing); tree assembly is `LCFormat.derivationToLaTeX(root, solution, {sentence})` so the harness and UI share it. Macros are `\cnode`/`\crule` (not `\den` — denotations are plain math). (b) `test/latex.mjs` + `test/golden-latex.txt` (1462 lines: every root term + one full forest per worksheet); `npm test` is now four stages; `test:update` refreshes both goldens. **The latexmk compile smoke test runs when latexmk is on PATH (it is in the dev sandbox — it caught a real bug: a naive 60-line sample slice cut a forest environment in half; the sampler now takes whole exports).** CI's ubuntu runner has no TeX → skips with notice. (c) Copy-LaTeX gate: teachers always; students when allDone and not assessment. (d) Deep links accept an optional `#worksheetKey/gid.pid` prefix (extension); hash written via replaceState (no history spam, no hashchange loops). Lingdown's `[[derivation:…]]` rule is registered BEFORE the `[[…]]` denotation rule. (e) Hints have no editor UI yet (W14 candidate) but round-trip losslessly through the editor (parseFromText/generateJSON passthrough).
...
**S11 as-built notes:** (a) Scratchpad reuses the editor's `__composeBuildSet` trial-set machinery and launches through the app's existing `custom`-set path; all rules + shifters + QR enabled; Promote → editor (instructors) / .json download (everyone); entries: Tools menu, `#scratchpad` hash, dash can deep-link. (b) SW registration happens from APP code on hosted STUDENT builds only (root + /v/*; explicitly not `hosted-teacher`) and injects the manifest `<link>` dynamically — the assembler stayed untouched. (c) **Toast deviation:** with network-first you are never knowingly stale while online, so the brief's "updated — reload" postMessage toast is replaced by online/offline notices ("Offline — using your saved copy"); the versioned cache name (from version.js) retires old caches on every deploy. (d) sw.js/manifest.json/icon.svg are generated by build:server into pb_public (gitignored like the rest). (e) Server suite → 65 checks (sw served+versioned+exclusion list, manifest, icon, registration marker in /v/*, dash provably NOT registering, scratchpad marker).
