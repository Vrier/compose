...
second-order properties that take a first-order property as argument (@gq).
...
predicate, and returns a truth value (@det).
...
exactly two cats, and *Neither cat has stripes* (@both) inherits that condition while
